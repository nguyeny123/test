...
_db = db;
    ...
    // } 
    [HttpGet]
    public async Task<IActionResult> Findss(string q, string brands, int? minPrice, int? maxPrice, int? minScreen, int? maxScreen, string capacity, string colours, string os, string features)
    {
      var Query = $"%{q?.ToLower()}%";
      ...
      var products = await _db.Products
        ...
        .Where(x => Capacity.Any() == false || x.ProductVariants.Any(v => Capacity.Contains(Int32.Parse(v.Storage.Capacity))))
    ...
    public async Task<IActionResult> Create([FromBody] CreateProductViewModel model)
    {
        ...
        foreach (var variant in model.Variants)
        {
          ...
          var capacity = Convert.ToString(variant.Storage.Substring(0,variant.Storage.IndexOf("GB")));
